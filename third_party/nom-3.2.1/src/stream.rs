...
pub trait Producer<'b,I,M: 'b> {
  ...
  /// `Consumer` chain
  fn apply<'a, O,E>(&'b mut self, consumer: &'a mut dyn Consumer<I,O,E,M>) -> &'a ConsumerState<O,E,M>;

  /// Applies a consumer once on the produced data, and returns the generated value if there is one
  fn run<'a: 'b,O,E: 'b>(&'b mut self, consumer: &'a mut dyn Consumer<I,O,E,M>)   -> Option<&'b O> {
...
impl<'x> MemProducer<'x> {
  pub fn new(buffer: &'x[u8], chunk_size: usize) -> MemProducer<'x> {
    MemProducer {
      buffer:     buffer,
...
impl<'x,'b> Producer<'b,&'x[u8],Move> for MemProducer<'x> {
  fn apply<'a,O,E>(&'b mut self, consumer: &'a mut dyn Consumer<&'x[u8],O,E,Move>) -> &'a ConsumerState<O,E,Move> {
    if {
  ...
  pub fn new(filename: &str, buffer_size: usize) -> io::Result<FileProducer> {
    File::open(&Path::new(filename)).and_then(|mut f| {
      f.seek(SeekFrom::Start(0)).map(|_| {
        ...
        FileProducer {file: f, position: 0, v: v, start: 0, end: 0, state: FileProducerState::Normal }
      })
    })
  }
...
impl<'x> Producer<'x,&'x [u8],Move> for FileProducer {
  ...
  fn apply<'a,O,E>(&'x mut self, consumer: &'a mut dyn Consumer<&'x[u8],O,E,Move>) -> &'a ConsumerState<O,E,Move> {
